...
class Tasks[F[_]](server: XlrServer)(implicit protected val _api: DSL[F]) extends XlrLib[F] with DefaultJsonProtocol {
  ...
  def appendScriptTask(phaseId: Phase.ID, title: String, taskType: String, script: String)
    ...
    for {
      _ <- log.debug(s"appendScriptTask(${phaseId.show}, $title, $taskType, $script)")
  ...
  def appendManualTask(phaseId: Phase.ID, title: String)
                      (implicit session: User.Session): Program[Task.ID] =
    for {
      _ <- log.debug(s"appendManualTask(${phaseId.show}, $title)")
      resp <- lib.http.json.post(server.api(_ / "tasks" / "Applications" / phaseId.release / phaseId.phase / "tasks"),
        JsObject(
          "id" -> JsNull,
          "title" -> title.toJson,
  ...
  def assignTo(taskId: Task.ID, assignee: User.ID)
    ...
    for {
      _ <- log.debug(s"assignTo(${taskId.show}, $assignee")
      resp <- lib.http.json.post(
        server.api(_ / "tasks" / "Applications" / taskId.release / taskId.phase / taskId.task / "assign" / assignee),
        JsNull
      )
  ...
  def complete(taskId: Task.ID, comment: Option[String] = None)
    ...
    for {
      _ <- log.debug(s"complete(${taskId.show}, $comment)")
      resp <- lib.http.json.post(
        server.api(_ / "tasks" / "Applications" / taskId.release / taskId.phase / taskId.task / "complete"),
  ...
  def waitFor(taskId: Task.ID,
             ...
             (implicit session: User.Session): Program[Unit] = {
    for {
      _ <- log.debug(s"waitFor(${taskId.show}, $expectedStatus, $interval, $retries")
  ...
  def getTaskStatus(taskId: Task.ID)
    ...
    for {
      _ <- log.debug(s"getTaskStatus(${taskId.show})")
  ...
  def getComments(taskId: Task.ID)
    ...
    for {
      _ <- log.debug(s"getComments(${taskId.show})")
      content <- lib.http.json.get(server.api(_ / "tasks" / "Applications" / taskId.release / taskId.phase / taskId.task))
